...
export const SCHEMA_DEFINITIONS: Record<SchemaCategory, SchemaTypeDefinition> = {
  SoftwareApplication: {
    ...
    fields: [
      ...
      { id: 'description', label: 'Description', type: 'textarea', placeholder: 'Ultra-fast real-time JSON-LD schema builder and AEO optimizer.', required: true, defaultValue: 'JSON-LD schema builder and AEO optimizer with in-browser validation and live SERP preview.' },
      ...
      { id: 'ratingValue', label: 'Rating (0-5)', type: 'number', placeholder: 'e.g. 4.6 if you display real reviews' },
      { id: 'ratingCount', label: 'Rating Count', type: 'number', placeholder: 'Only if reviews are visible on-page' },
    ...
    defaultValues: {
      ...
      description: 'JSON-LD schema builder and AEO optimizer with in-browser validation and live SERP preview.',
    ...
    generateJsonLd: (val) => ({
      ...
      aggregateRating: val.ratingValue && val.ratingCount ? {
        '@type': 'AggregateRating',
        ratingValue: String(val.ratingValue),
        reviewCount: String(val.ratingCount),
        bestRating: '5',
        worstRating: '1',
      } : undefined,
      author: {
  ...
  Product: {
    ...
    fields: [
      ...
      { id: 'ratingValue', label: 'Average Rating', type: 'number', placeholder: 'Only if reviews are visible on-page' },
    ...
    defaultValues: {
      faqs: [
        ...
        },
        {
          ...
          answer: 'Google recrawls on its own schedule. Use Search Console URL Inspection to request a recrawl. There is no guaranteed discovery window.',
        },
        {
          question: 'Does SchemaCraft validate against Schema.org and Google Search Console specs?',
          answer: 'Yes. SchemaCraft checks JSON-LD in your browser against documented required and recommended properties. Google does not guarantee that valid markup produces a rich result.',
        },
      ],
    },
    generateJsonLd: (val) => {
  ...
  Organization: {
    ...
    fields: [
      ...
      { id: 'description', label: 'Corporate Bio', type: 'textarea', placeholder: 'Enterprise data architecture and automated schema generation platform.', defaultValue: 'Enterprise data architecture and automated schema generation platform providing in-browser AEO tooling.' },
    ...
    defaultValues: {
      ...
      description: 'Enterprise data architecture and automated schema generation platform providing in-browser AEO tooling.',
